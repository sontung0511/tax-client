...
function Transactions({ data, refresh }: { data: TaxDatabase; refresh: () => Promise<void> }) {
  const [amount, setAmount] = useState(""); const [vatAmount, setVatAmount] = useState(""); const [date, setDate] = useState(currentDateInputValue); const [description, setDescription] = useState(""); const [type, setType] = useState<Transaction["type"]>("revenue"); const [category, setCategory] = useState<BusinessProfile["industry"]>(data.profile.industry);
  ...
  async function submit(e: FormEvent) {
    ...
    if (!parseVnd(amount) || !description.trim() || !isValidDateInput(date)) { setMessage("Vui lòng nhập đủ ngày, nội dung và số tiền hợp lệ."); return; }
    try {
      await taxApi.createTransaction({ id: crypto.randomUUID(), periodId: data.periods[0].id, date, type, description: description.trim(), invoiceNo: `HD-${Date.now().toString().slice(-5)}`, amount: parseVnd(amount), vatAmount: parseVnd(vatAmount), revenueCategory: category, paymentStatus: "paid", outstandingAmount: 0 });
      setAmount(""); setVatAmount(""); setDescription(""); await refresh();
    } catch (cause) { setMessage(cause instanceof Error ? cause.message : "Không thể lưu giao dịch"); }
  }
  ...
  return <><Header eyebrow="Giao dịch" title="Doanh thu, chi phí & hóa đơn" description={`Đang nhập dữ liệu cho ${data.periods[0].label}. Số tiền giao dịch và tiền thuế GTGT được lưu riêng theo số nguyên VND.`}/>{issues.length > 0 && <div className="mb-5 rounded-2xl border border-amber-200 bg-amber-50 p-4 text-sm text-amber-800">Có {issues.length} cảnh báo thiếu, sai hoặc trùng dữ liệu cần rà soát.</div>}{message && <p className="mb-5 rounded-xl bg-red-50 p-3 text-sm text-red-700">{message}</p>}<form onSubmit={submit} className="card mb-6 grid gap-4 md:grid-cols-2 lg:grid-cols-4"><Field label="Ngày giao dịch"><input required type="date" className="field" value={date} onChange={(e) => setDate(e.target.value)}/></Field><Field label="Loại giao dịch"><select className="field" value={type} onChange={(e) => setType(e.target.value as Transaction["type"])}><option value="revenue">Doanh thu</option><option value="expense">Chi phí</option></select></Field><Field label="Nhóm ngành"><select className="field" value={category} onChange={(e) => setCategory(e.target.value as BusinessProfile["industry"])}><option value="distribution">Phân phối</option><option value="services">Dịch vụ</option><option value="production">Sản xuất, vận tải</option><option value="other">Nhóm khác</option></select></Field><Field label="Nội dung"><input required className="field" placeholder="Nội dung giao dịch" value={description} onChange={(e) => setDescription(e.target.value)}/></Field><Field label="Số tiền (VND)"><input required className="field" inputMode="numeric" placeholder="0" value={amount} onChange={(e) => setAmount(e.target.value)}/></Field><Field label="Tiền thuế GTGT (VND)"><input className="field" inputMode="numeric" placeholder="0" value={vatAmount} onChange={(e) => setVatAmount(e.target.value)}/></Field><div className="flex items-end lg:col-span-2"><button disabled={Boolean(data.periods[0].lockedAt)} className="primary w-full disabled:opacity-40">Thêm mới</button></div></form><section className="card overflow-x-auto"><table><thead><tr><th>Ngày</th><th>Loại</th><th>Nhóm ngành</th><th>Nội dung</th><th>Hóa đơn</th><th className="text-right">Số tiền</th><th className="text-right">Thuế GTGT</th><th></th></tr></thead><tbody>{items.map((item) => <tr key={item.id} className={issues.some(issue => issue.transactionId === item.id) ? "bg-amber-50/50" : ""}><td>{editingDateId === item.id ? <input aria-label={`Ngày của ${item.description}`} type="date" className="field min-w-36" value={editingDate} onChange={(e) => setEditingDate(e.target.value)}/> : new Date(`${item.date}T00:00:00`).toLocaleDateString("vi-VN")}</td><td><span className={`badge ${item.type === "revenue" ? "bg-emerald-50 text-emerald-700" : "bg-slate-100 text-slate-600"}`}>{item.type === "revenue" ? "Doanh thu" : "Chi phí"}</span></td><td>{item.revenueCategory}</td><td className="font-medium text-ink">{item.description}</td><td>{item.invoiceNo}</td><td className="text-right font-semibold">{formatVnd(item.amount)}</td><td className="text-right">{formatVnd(item.vatAmount ?? 0)}</td><td>{editingDateId === item.id ? <div className="flex gap-2"><button disabled={Boolean(data.periods[0].lockedAt)} onClick={() => void saveDate(item)} className="link-button">Lưu</button><button onClick={() => setEditingDateId("")} className="text-xs text-slate-500">Hủy</button></div> : <button disabled={Boolean(data.periods[0].lockedAt)} onClick={() => { setEditingDateId(item.id); setEditingDate(item.date); setMessage(""); }} className="link-button disabled:opacity-40">Sửa ngày</button>}</td></tr>)}</tbody></table></section></>;
}
